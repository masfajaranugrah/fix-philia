...
const Maps = () => {
  ...
  return (
    <motion.div
      className="container w-full mx-auto px-6 flex my-10 flex-col justify-center items-center"
    ...
    >
        ...
        <motion.p 
          ...
          className=" text-center text-[24px] my-4 md:text-[40px] xl:text-[40px] text-[#2D210A] font-[Montserrat] font-[1000] "
        >
          PETA PHILIA
        </motion.p>
      </div>
      <motion.div
        className="border border-black rounded-[14px] xl:rounded-[35px] w-full  cursor-pointer overflow-hidden"
        initial={{ opacity: 0, y: 50 }}
        whileInView={{ opacity: 1, y: 0 }}
      ...
      >
        <motion.img
          src="/images/Maps.png"
          alt="maps"
          className="w-full h-80 block  xl:hidden lg:hidden md:hidden  sm:h-[60vh] xl:h-full lg:h-full md:h-full"
        ...
        />
         <motion.img
          src="/images/map.png"
          alt="maps"
          className="w-full h-80 hidden  xl:block lg:block md:block sm:h-[60vh] xl:h-full lg:h-full md:h-full"
          style={{ scale, position: "relative" }} // Tambahkan position: "relative"
          drag={scale > 1} // Bisa digeser jika diperbesar
          onDoubleClick={handleDoubleClick}
          onDragEnd={handleDragEnd} // Simpan posisi terakhir atau reset
